...
const JobCreateDialog = ({
  ...
  const [disabledPartnerField, setDisabledPartner] = useState(false);
  const [disabledCustomerField, setDisabledCustomer] = useState(false);
  
  // if(initialValues.partnerId !== null)
  // {
      ...
      >
        {({ values, handleChange, setFieldValue }) => {
          ...
          const handleJobTypeChange = (e) => {
            ...
            if (jobType && jobType.partnerId !== null) {
              setDisabledPartner(true);
              setFieldValue("partnerId", jobType.partnerId);
              // reset customer name, it is only for jobs with no partner
          ...
          const handleContractorChange = (e, index) => {
            ...
            const contractor = fullContractors.find(
              (c) => c.id === contractorId
            );
            const jobTypeId = values.jobTypeId;

            const jobType = fullJobTypes.find((j) => j.id === jobTypeId);
            const jobCategoryId = jobType.jobCategoryId;
            ...
            if (contractor) {
              ...
              if (
                ...
                contractor.contractorRates[jobTypeId]
              ) {
                defaultPay = contractor.contractorRates[jobTypeId];
              }
              // Or fallback to base rate
              else if (
                contractor.roleRates &&
                contractor.roleRates[jobCategoryId]
              ) {
                defaultPay = contractor.roleRates[jobCategoryId];
              } else {
                defaultPay = 0;
              }
            }
          ...
          const handlePartnerIdChange = (e) => {
            ...
            setFieldValue("partnerId", selectedPartnerId);
            handleChange(e);
            ...
            <Form>
              ...
              <DialogActions>
                ...
                <Button
                  ...
                  disabled={loading}
                  sx={{
                    backgroundColor: colors.burntOrange[600],
                    color: "#fff",
                  }}
                >
